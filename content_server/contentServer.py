...
x,sr = librosa.load('./mock_file_storage/audio_store/enc.wav', sr=44100)
...
with open('./mock_file_storage/temp/temp.txt', 'wb') as f:
    f.write(tb)
...
while True:
    conn, addr = s.accept() 
    f = open('./mock_file_storage/temp/temp.txt','rb')
    l = f.read(1024)
    while (l):
        conn.send(l)
        l = f.read(1024)
...
conn.close()
s.close()
os.remove('./mock_file_storage/temp/temp.txt')